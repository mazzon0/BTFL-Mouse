...
- While in START state, `enter_standard_mode()` function enables the device to work in high performance mode, at a maximum frequence of 240MHz. After 5 minutes of inactivity, the device will go in the LOW_POWER_CONSUMPTION state and the `enter_low_power_mode(gpio_isr_t isr)` function will lower CPU frequency to a maximum of 80MHz and disable the Touch Sensor.
